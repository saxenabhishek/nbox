...
1. `get_logger`: a master logger for nbox, this can be modified to log through anything
1. `isthere`: a decorator that checks if a package is installed, if not it raises an error\
...
1. `get_files_in_folder`: a function that returns all files in a folder with a certain extension
1. `fetch`: a function that fetches a url and caches it in `tempdir` for faster loading
1. `get_random_name`: a function that returns a random name, if `True` is passed returns\
  an `uuid4()` for truly random names :)
1. `hash_`: a function that returns a hash of any python object, string is accurate, others\
  might be anything, but atleast it returns something.
1. `folder/join`: to be used in pair, `join(folder(__file__), "server_temp.jinja")` means\
  relative path "../other.py". Scattered throughout the codebase, the super generic name will\
  bite me.
1. `to_pickle/from_pickle`: to be used in pair, `to_pickle(obj, "path")` and `from_pickle("path")`\
  to save and load python objects to disk.
1. `DBase`: सस्ता-protobuf (cheap protobuf), can be nested and `get_dict` will get for all\
  children
1. `PoolBranch`: Or how to use multiprocessing, but blocked so you don't have to give a shit\
  about it.
"""
...
class env:
  ...
  1. `NBOX_LOG_LEVEL`: Logging level for `nbox`, set `NBOX_LOG_LEVEL=info|debug|warning"
  1. `NBOX_JSON_LOG`: Whether to print json-logs, set `NBOX_JSON_LOG=1`
  1. `NBOX_JOB_FOLDER`: Folder path for the job, set `NBOX_JOB_FOLDER=/tmp/nbox/jobs"`
  1. `NBOX_NO_AUTH`: If set `secrets = None`, this will break any API request and is good only for local testing
  1. `NBOX_SSH_NO_HOST_CHECKING`: If set, `ssh` will not check for host key
  1. `NBOX_HOME_DIR`: By default ~/.nbx folder
  1. `NBOX_USER_TOKEN`: User token for NimbleBox.ai from <app.nimblebox.ai/secrets>_
  1. `NBOX_NO_LOAD_GRPC`: If set, will not load grpc stub
  1. `NBOX_NO_LOAD_WS`: If set, will not load webserver subway
  1. `NBOX_LMAO_DISABLE_RELICS`: If set, Monitoring data will be stored on the cloud Relic
  1. `NBOX_LMAO_DISABLE_SYSTEM_METRICS`: If set, system metrics will not logged in monitoring
  """
